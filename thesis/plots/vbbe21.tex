...
\centering\begin{tikzpicture}[node distance=0cm,start chain=1 going right] \footnotesize
  \tikzstyle{mytape}=[draw,minimum height=1.7cm]
	...
	\node [on chain=1,mytape,fill=gray!35] {$\underset{\text{boundary}}{\underbrace{\overbracket{\text{ }\dots \text{ }}^{<1\text{ byte}}}_{\text{fill to byte}}}$};
...
\end{tikzpicture}
	\caption{\label{fig:vbbe21}The compact vbbe21 encoding bit packs the
	number of exceptions, the deltas of the exceptions' positions and the
	two byte exceptions subtracted by 256. Less than one byte is used to
	fill the bit packed data to the next byte boundary. Then, the one byte
	data is recorded as in vbe21.}
\end{figure}
