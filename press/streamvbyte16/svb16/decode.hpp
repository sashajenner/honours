...
size_t decode(bool UseDelta, bool UseZigzag, int16_t *out, uint32_t out_size, uint8_t const *in, int16_t prev)
{
...
#ifdef SVB16_X64
    if (has_sse4_1()) {
        return decode_sse(UseDelta, UseZigzag, out, out_size, keys, keys_length, data, prev) - in;
    }
#endif
    return decode_scalar(UseDelta, UseZigzag, out, out_size, keys, keys_length, data, prev) - in;
}
