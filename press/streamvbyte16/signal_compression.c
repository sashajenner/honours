        ...
        uint8_t *out)
{
    ...
    const uint64_t encoded_count = encode(usedelta, usezigzag, in, out, nin, 0);
...
void decompress_signal_nozstd(
        bool zd,
        const uint8_t *in,
        uint64_t nin, /* nin in original in */
        int16_t *out)
{
    bool usedelta = false;
    bool usezigzag = false;
    ...
    /*size_t consumed_count =*/ (void) decode(usedelta, usezigzag, out, nin, in, 0);

    /*if (consumed_count != nin) {*/
        /* Remaining data at end of signal buffer */
    /*
        out = NULL;
    }
