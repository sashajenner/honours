...
/*[[gnu::target("ssse3")]]*/ inline __m128i load_8(bool UseDelta, bool UseZigzag, int16_t const *from, __m128i *prev) {
...
/*[[gnu::target("ssse3")]]*/ uint8_t *encode_sse(bool UseDelta, bool UseZigzag, int16_t const *in,
    ...
    constexpr __m128i const mask_01 = m128i_from_bytes(0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
                                                  0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01);
    for (const int16_t *end = &in [(count & ~15)]; in != end; in += 16) {
        ...
        // 1 byte per input int16_t: FF if the MSB is set, 00 or 01 if not
        // (us = unsigned saturation)
        r2 = _mm_packus_epi16(r2, r3);
        // 1 bit per input int16_t: 1 if the MSB is set, 0 if not
